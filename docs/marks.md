...
<!-- 个别字体比如 Open Sans，按 shift 加减号键一次打出的符号已经占两个汉字长度，没必要打两遍 -->
...
（3）数值范围（例如日期、时间或数字）应该使用全形波浪号（`～`）或中文半破折号（`—`），占一个全角字符的位置。
<!-- 全形波浪号并非`~`（U+007E），而是（U+FF5E）。
 中文半破折号就是中文状态下按 shift 加减号键，再回退删除一半 -->
